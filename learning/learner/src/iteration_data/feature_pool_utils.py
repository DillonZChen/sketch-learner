...
def compute_dlplan_feature_pool(
    ...
    if not disable_feature_generation:
        ...
        for numerical in generated_numericals:
            features.append(DLPlanFeature(numerical, numerical.compute_complexity() + 1))
        for boolean in generated_booleans:
            features.append(DLPlanFeature(boolean, boolean.compute_complexity() + 1 + 1))
